			...
			{articles.map(({ id, title }) => (
				<div className="flex flex-col" key={id}>
					<Link to={`/articles/${id}`} className="flex-1 h-full">
						<div className="flex-1 bg-navy-600 rounded text-white p-6 min-h-fit h-full flex flex-col justify-between space-y-2 transition-all drop-shadow-sm hover:drop-shadow-xl hover:scale-105">
							<h2>{title}</h2>
						</div>
					</Link>
				</div>
